...
const user = ref({
    ...
    phone: "",
    about: "",
    photo: "https://i.pravatar.cc/150?img=3" // can be URL or File
});
...
const filtered = computed(() => {
    ...
    if (q) {
        arr = arr.filter((d) =>
            [d.name, d.email, d.role, d.designation, d.phone]
                .filter(Boolean)
...
async function fetchList() {
    ...
    try {
        ...
        toast({
            type: "error",
            title: "Load failed",
            message: e?.response?.data?.message || "Could not load users.",
        });
    } finally {
        loading.value = false;
    }
...
async function submit() {
    ...
    try {
        ...
        const { data } = await api.post("/admin/users", fd);

        items.value.unshift(data.User ?? data);
        resetForm();
        showAdd.value = false;
        toast({
            type: "success",
        ...
        if (e?.response?.status === 422) {
            ...
            toast({
                ...
                title: "Create failed",
                message:
                    e?.response?.data?.message || "Could not create User.",
            });
        }
    } finally {
...
async function saveEdit() {
    ...
    try {
        ...
        const { data } = await api.post(
            `/admin/users/${editing.value.id}`,
            fd
        );

        const idx = items.value.findIndex((x) => x.id === editing.value.id);
        if (idx > -1) items.value[idx] = data.User ?? data;

        showEdit.value = false;
        toast({ type: "success", title: "Saved", message: "User updated." });
    } catch (e) {
...
if (editForm.photo && typeof editForm.photo !== 'string') {
    URL.revokeObjectURL(editForm.photo);
}
</script>

<template>
    <AuthenticatedLayout title="Admin · Users">
        <div class="p-4 sm:p-6 max-w-[1680px] mx-auto">
            <!-- Header -->
            <header
            ...
            >
                <div>
                    ...
                    <p class="text-gray-600 mt-1">
                        Only existing users are listed below.
                    </p>
                </div>
                            ...
                            <td class="px-2 py-3 sm:px-4">
                                ...
                                >
                                    <button
                                        ...
                                        @click="openProfile(d)"
                                    >
                                        👁️ View Profile <span class="sr-only">View</span>
                                    </button>
<!--                                    <button
                        ...
                        <tr v-if="!loading && filtered.length === 0">
                            ...
                            >
                                ...
                                <p class="text-xs sm:text-sm">
                                    Use the Add User button to create a new
                                    profile.
                                </p>
                            </td>
                        </tr>
            ...
            <!-- USER PROFILE MODAL (TWO COLUMNS) -->
            <Teleport to="body">
                <Transition name="fade">
                    <div
                        v-if="showProfile"
                    ...
                    >
                        ...
                        <div
                            role="dialog"
                            aria-modal="true"
                            class="relative w-full max-w-2xl rounded-2xl bg-white border border-gray-200 shadow-xl mx-4 sm:mx-0 overflow-hidden"
                        >
                            <!-- header -->
                            ...
                            <div class="p-6">
                                <div class="flex flex-col sm:flex-row sm:gap-6">
                                    ...
                                    <div class="flex-shrink-0 flex justify-center mb-4 sm:mb-0">
                                        ...
                                        />
                                        <div v-else class="w-28 h-28 rounded-full bg-gray-100 flex items-center justify-center text-gray-400 text-3xl">
                                            👤
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-gray-900">{{ user.email || "Not provided" }}</div>
                                        </div>

                                        <div>
                                            <label class="block text-sm font-medium text-gray-500">Role</label>
                                            <div class="text-gray-900">{{ user.role || "Not specified" }}</div>
                                        </div>

                                        <div>
                                            <label class="block text-sm font-medium text-gray-500">Designation</label>
                                            <div class="text-gray-900">{{ user.designation || "Not specified" }}</div>
                                        </div>
                                        ...
                                        <div class="sm:col-span-2">
                                            ...
                                            <div class="text-gray-900">{{ user.about || "No information available" }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
