    ...
    <div class="page">
            ...
            <table class="brand-row">
                <tr>
                    ...
                    <td style="text-align: right;">
                        <span class="pill">{{ strtoupper((string) $quotation->status) }}</span>
                    </td>
                </tr>
            </table>
                ...
                <tr>
                    <td width="33.33%">
                        <div class="meta-card">
                            ...
                            <div class="card-value">{{ $quotation->quotation_number }}</div>
                        </div>
                    </td>
                    <td width="33.33%">
                        <div class="meta-card">
                            <div class="card-label">Issue Date</div>
                            <div class="card-value">{{ optional($quotation->issue_date)->format('d M Y') }}</div>
                        </div>
                    </td>
                    <td width="33.33%">
                        <div class="meta-card">
                            <div class="card-label">Expiry Date</div>
                            <div class="card-value">{{ optional($quotation->expires_at)->format('d M Y') }}</div>
                        </div>
                    </td>
                </tr>
            </table>
            ...
            <tr>
                <td width="48%">
                    <div class="section-card">
                        ...
                        <div class="card-value" style="margin-top: 0;">{{ data_get($quotation->customer_snapshot, 'name', 'Customer') }}</div>
                        <div class="muted" style="margin-top: 8px;">{{ data_get($quotation->customer_snapshot, 'email', 'No email recorded') }}</div>
                        <div class="muted" style="margin-top: 4px;">{{ data_get($quotation->customer_snapshot, 'phone', 'No phone recorded') }}</div>
                    </div>
                </td>
                <td width="4%"></td>
                <td width="48%">
                    <div class="section-card">
                        ...
                        <div class="card-value" style="margin-top: 0;">{{ $quotation->pickupLocation?->name ?? data_get($quotation->route_snapshot, 'pickup_city', 'Pickup') }} to {{ $quotation->dropoffLocation?->name ?? data_get($quotation->route_snapshot, 'dropoff_city', 'Destination') }}</div>
                        <div class="muted" style="margin-top: 8px;">{{ number_format((float) $quotation->distance_km, 2) }} km operational distance</div>
                        <div class="muted" style="margin-top: 4px;">Estimated travel {{ number_format((float) $quotation->estimated_hours, 1) }} hours</div>
                    </div>
                </td>
            </tr>
        </table>

        @if($quotation->driver_snapshot)
            <div class="section-card">
                <div class="section-title">Suggested Driver</div>
                <table class="detail-table">
                    <tr>
                        <td class="muted">Driver</td>
                        <td>{{ data_get($quotation->driver_snapshot, 'name', 'Selected driver') }}</td>
                    </tr>
                    <tr>
                        <td class="muted">Verification</td>
                        <td>{{ ucfirst(str_replace('_', ' ', (string) data_get($quotation->driver_snapshot, 'verification_status', 'pending'))) }}</td>
                    </tr>
                    <tr>
                        <td class="muted">Vehicle</td>
                        <td>{{ data_get($quotation->driver_snapshot, 'vehicle_label', 'Vehicle on file') }}</td>
                    </tr>
                    <tr>
                        <td class="muted">Route coverage</td>
                        <td>{{ data_get($quotation->driver_snapshot, 'route_summary', 'Operational route coverage available') }}</td>
                    </tr>
                </table>
            </div>
        @endif
        ...
        <div class="section-card">
            ...
            <table class="detail-table">
                <tr>
                    ...
                    <td>{{ $quotation->packageType?->name ?? 'Parcel' }}</td>
                </tr>
                <tr>
                    <td class="muted">Weight</td>
                    <td>{{ number_format((float) $quotation->weight_kg, 2) }} kg</td>
                </tr>
                <tr>
                    <td class="muted">Urgency</td>
                    <td>{{ ucfirst(str_replace('_', ' ', (string) $quotation->urgency_level)) }}</td>
                </tr>
                <tr>
                    <td class="muted">Status</td>
                    <td>{{ ucfirst(str_replace('_', ' ', (string) $quotation->status)) }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="section-card">
            ...
            <table class="breakdown-table">
                <tr><td class="muted">Base fee</td><td>N$ {{ number_format((float) $quotation->base_fee, 2) }}</td></tr>
                <tr><td class="muted">Distance fee</td><td>N$ {{ number_format((float) $quotation->distance_fee, 2) }}</td></tr>
                <tr><td class="muted">Weight fee</td><td>N$ {{ number_format((float) $quotation->weight_fee, 2) }}</td></tr>
                <tr><td class="muted">Urgency fee</td><td>N$ {{ number_format((float) $quotation->urgency_fee, 2) }}</td></tr>
                <tr><td class="muted">Special handling</td><td>N$ {{ number_format((float) $quotation->special_handling_fee, 2) }}</td></tr>
            </table>
            ...
            <div class="total-box">
                <table class="total-row">
                    <tr>
                        ...
                        <td>N$ {{ number_format((float) $quotation->total, 2) }}</td>
                    </tr>
                </table>
            </div>
        </div>
